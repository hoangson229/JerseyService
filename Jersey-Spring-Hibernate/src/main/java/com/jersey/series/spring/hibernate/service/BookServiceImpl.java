...
public class BookServiceImpl implements IBookService {
	...
	public BookType getBookInfo(@PathParam("id") int bookId) {
		...
		// retrieve book information based on the id supplied in the formal
		// argument
	...
	// http://localhost:8080/Jersey-Spring-Hibernate/rest/bookservice/getbook/1
	@GET
	@Path("getbook/json/{id}")
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.APPLICATION_JSON)
	public Response getBookInfoJson(@PathParam("id") int bookId) {
		...
		bookType.setAuthor(getBook.getAuthor());
		bookType.setCategory(getBook.getCategory());
		return Response.status(200).entity(bookType).build();
	...
	public BookListType getAllBookInfo() {
		...
		// create a object of type BookListType which takes book objects in its
		// list
